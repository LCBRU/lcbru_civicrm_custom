...
class CRM_Activity_BAO_ICalendar {
   ...
   * @param string $case_type_id Case Type ID of the parent case for any activity that is case-specific
   
   *
   * @return string|null
   *   Array index of the added attachment in the $attachments array, else NULL.
   */
  public function addAttachment(&$attachments, $contacts, $caseType = NULL) {
    // Check preferences setting

    $xmlProcessorProcess = new CRM_Case_XMLProcessor_Process();
    $isNotifyIcal = $xmlProcessorProcess->getCaseTypeActivityNotificationsIcal($caseType);

    //if (Civi::settings()->get('activity_assignee_notification_ics')) {
    if ( CRM_Core_BAO_Setting::getItem( CRM_Core_BAO_Setting::SYSTEM_PREFERENCES_NAME, 'activity_assignee_notification_ics' ) || $isNotifyIcal ) {
      ...
      if ($this->icsfile !== FALSE) {
        ...
        $calendar = str_replace("\n","\r\n",$calendar);   //  Changing line ending to Windows style to allow inline attachment.
        if (file_put_contents($this->icsfile, $calendar) !== FALSE) {
          ...
          $attachments['activity_ics'] = array(
            ...
            'disposition' => 'inline', // or 'attachment', // results in iCal attachment being rendered inline. Maybe.
          );
          return 'activity_ics';
        }
      }
