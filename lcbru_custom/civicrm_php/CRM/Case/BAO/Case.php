 ...
 */
 ...
 /**
  *
  * With customisations for LCBRU by NRH to add more variables into the template for emails sent to assigned users for activities
  *
  */
 ...
 LEFT JOIN  civicrm_email ON (civicrm_email.contact_id = con.id AND civicrm_email.is_primary = 1)
  ...
  public static function sendActivityCopy($clientId, $activityId, $contacts, $attachments = NULL, $caseId) {
    ...
    foreach ($tplParams['activity']['fields'] as $k => $val) {
      ...
      if (CRM_Utils_Array::value('label', $val) == ts('Medium')) {
        $activityMedium = $val['value'];
      }
      if (CRM_Utils_Array::value('label', $val) == ts('Location')) {
        $activityLocation = $val['value'];
      }
      if (CRM_Utils_Array::value('label', $val) == ts('Activity Type')) {
    ...
    // Put some variables into the template for the assignee email  
    $tplParams['activitySubject'] = $activitySubject;
    $tplParams['activityMedium'] = $activityMedium;
    $tplParams['activityLocation'] = $activityLocation;
    $tplParams['activityType'] = $activityType;
    $tplParams['activityClient'] = $tplParams['activity']['fields']['0']['value'];


    // if it’s a case activity, add hashed id to the template (CRM-5916)
    if ($caseId) {
      $tplParams['caseId'] = $caseId;
